...
#ifndef CPP_BASE_HASH_HASH_H_
#define CPP_BASE_HASH_HASH_H_
...
struct hash<std::pair<First, Second> > {
  size_t operator()(const std::pair<First, Second>& p) const {
    ...
    return (sizeof(h1) <= sizeof(uint32))
               ?  // NOLINT
               cpp_base::Hash32NumWithSeed(h1, h2)
               : cpp_base::Hash64NumWithSeed(h1, h2);
  }
};
...
struct hash<std::thread::id> {
    size_t operator()(const std::thread::id& id) const { return std::hash<std::thread::id>()(id); }
};
