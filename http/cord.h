...
#ifndef CPP_BASE_HTTP_CORD_H_
#define CPP_BASE_HTTP_CORD_H_
...
class Cord {
 public:
  typedef std::deque<CordBuffer>::iterator iterator;
  typedef std::deque<CordBuffer>::const_iterator const_iterator;
  ...
  // Return the total amount of space in the Cord which has been allocated but
  // not used (and may be unavailable for use)
  ...
  inline void CopyFrom(const std::string &str) {
    CopyFrom(str.data(), str.size());
  }

  // Append a block to the Cord. Copies of the data are not made to all care
  // must be made not to delete the underlying objects before the Cord is
  // deleted.
  ...
  void Substr(uint64_t start, uint64_t len, std::string *out) const;
  std::string Substr(uint64_t start, uint64_t len) const;

  // Return a string containing the first full line (terminated by [\r\n]) in
  // the Cord. Throws out_of_range if there is no complete line available.
  void ConsumeLine(std::string *output);

  // Return a string containing the first <butes> bytes of the Cord.
  // Throws out_of_range if there are not enough bytes available.
  void Consume(uint64_t bytes, std::string *output);
