...
TEST_F(CordTest, Append) {
  Cord cord;
  string teststring("jfdkjasxxxxhilfuhawelsuehfliwnevlkasuhvjnvkjlawekrnlkasuhi"
                    "awhvkjahlkjskljhflwkevcnlkjsZNLDvsdx\n");
  for (int i = 0; i < 20; i++) {
...
TEST_F(CordTest, Consume) {
  ...
  cord.CopyFrom("\r\n");  // empty line
  cord.CopyFrom("This is line 2, it is a long line that spans "
                "multiple CordBuffer blocks.\r\n");
...
TEST_F(CordTest, ConsumeLine) {
  ...
  cord.CopyFrom("\r\n");  // empty line
  cord.CopyFrom("This is line 3, it is a long line that spans "
                "multiple CordBuffer blocks.\r\n");
  ...
  output.clear();
  cord.ConsumeLine(&output);
  EXPECT_EQ("This is line 3, it is a long line that spans "
            "multiple CordBuffer blocks.", output);
  output.clear();
