...
char Cord::at(uint64_t start) const {
  ...
  LOG(FATAL) << "Cord::at(" << start << ") failed to find a character";
}
...
void Cord::ConsumeLine(string *output) {
  ...
  if (output) {
    int trim_chars = 0;
    for (string::reverse_iterator i = output->rbegin();
         i != output->rend(); ++i) {
      if (*i == '\n' || *i == '\r')
        trim_chars++;
      else
...
void Cord::Consume(uint64_t bytes, string *output) {
  ...
  CHECK_EQ(bytes_left, 0)
        << "Something went wrong; wrong num bytes returned from Cord::Consume";
}
