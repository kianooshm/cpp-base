...
void HttpMessage::set_http_version(const string &version) {
  // std::regex regex("(?i)HTTP/(\\d+)\\.(\\d+)");
  std::regex regex("HTTP/(\\d+)\\.(\\d+)");
  std::smatch what;
  if (!std::regex_match(version, what, regex) || what.size() != 3) {
...
void HttpMessage::SetContentLength(size_t length) {
  if (length == 0)
    length = body_.size();
  headers_.SetHeader("Content-Length", StrCat(length));
}
...
uint64_t HttpMessage::GetContentLength() const {
  ...
  if (!header.size())
    return 0;
  uint64 res;
  CHECK(safe_strtou64(header, &res)) << header;
  return res;
}
...
void HttpMessage::WriteChunk(Socket *sock, const StringPiece &chunk) {
  ...
  sock->Write("\r\n");
  sock->Write(chunk.data(), chunk.size());
  sock->Write("\r\n");
}
...
void HttpMessage::ReadAndParseHeaders(Socket *sock, int deadline_ms) {
  for (size_t i = 0; i < 5000; ++i) {
    ...
    try {
      ...
      try {
        sock->Read(deadline_ms);
        continue;
      } catch (exception) {
        throw runtime_error(StringPrintf("No response received from %s",
    ...
    if (output.at(0) == ' ' || output.at(0) == '\t') {
      ...
      headers_.AddHeader(output.substr(0, pos),
                         StringTrim(output.substr(pos + 1)));
    }
  }
