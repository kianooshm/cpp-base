...
std::regex url_regex("^(?:(\\w+)://)?"  // protocol
                       "(?:([^@:]+)(?::([^@]+))?@)?"  // username & password
                       "([^/]+?)?"        // hostname
                       "(?::(\\d+))?"
                       "(/[^\\?\\&]*)?"   // path
...
bool Uri::UrlEncode(const string &in, string *out) {
  out->clear();
  // Output string could potentially contain 3x the amount of characters as
  // the input.
...
static char HexToChar(const string &in) {
  char out;
...
const string &Uri::GetParam(const string &key) const {
  ...
  for (vector<Uri::RequestParam>::const_iterator i = params.begin();
       i != params.end(); ++i) {
    if (i->key == key)
      return i->value;
  }
